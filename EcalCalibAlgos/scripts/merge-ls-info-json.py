...
if __name__ == "__main__":
    ...
    cmd_DAS_query = subprocess.Popen(
        ["dasgoclient -query='file dataset="+options.dataset+" instance=prod/phys03 ' -limit 0 | grep '/store/'"],stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
    ...
    if pu_file_str == "":
	cmd_runs = subprocess.Popen(["dasgoclient -query='run dataset=" + options.dataset + " instance=prod/phys03' -limit=0"], stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True)
